    ...
    var getOAuthInfo = function(userobj, cb) {
        ...
        if (!(userobj.oauth && userobj.oauth.consumer_keys && userobj.oauth.consumer_keys[username] && userobj.oauth.tokens && userobj.oauth.tokens.node_sign_token)) {
            ...
            cb({
                ...
                "consumer_secret": userobj.oauth.consumer_keys[username],
                "token_key": "node_sign_token",
                "token_secret": userobj.oauth.tokens.node_sign_token
            });
        }
    }

    var setOAuthInfo = function(authInfo) {
        $("#consumer_key").val(authInfo.consumer_key);
        $("#consumer_secret").val(authInfo.consumer_secret);
        $("#token_secret").val(authInfo.token_secret);
        $("#node_url").val(window.location.protocol+"//"+window.location.host);
