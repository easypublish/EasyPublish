	...
	var List = Backbone.View.extend({
				...
				initialize: function () {
					...
					function ellipsis() {
						...
						that.$el.find(".description").ellipsis({row:3});
					}
					this.listenTo(Items, 'add', ellipsis);
					this.listenTo(this, 'render', ellipsis);
				},
				render: function () {
					if (!storedCredentials) {
						...
						var count = new Count();
						count.render();
						this.$el.find(".description").ellipsis({row:3});
					}
				},
				events: {
				...
				sync: function (event) {
					event.preventDefault();
					$.ajax('http://lrdev.local/extract/ezpublish-submitters/resource-by-discriminator?discriminator=[%22c8c0bf5288f4497ca683ba3111bc6800%22,%22pam.griffith@sri.com%22]',{
						type: 'GET',
						dataType: 'json',
				...
				exportData: function(event) {
					...
					if ("ezcsv" == format) {
						...
						eData = _.map(colHeads, quoted).join(",") + "\n";
