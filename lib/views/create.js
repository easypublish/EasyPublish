	...
	exports.Create = Backbone.View.extend({
				...
				render: function () {
					if (!storedCredentials) {
						...
						this.easyPub.buildForm("a11y", this.easyPub.fieldManager.a11yFields);
				...
				publish: function (event) {
					...
					if(this.easyPub.validateAll()) {
						var view = this;
						var data = this.easyPub.dataManager.makeAllEnvelopes(true);
					...
					if (window.File && window.FileReader && window.FileList && window.Blob) {
						if(this.easyPub.validateAll()) {
							...
							if (this.item) {
								...
								else if (this.item.get('status') === 'Error') {
